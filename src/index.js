 ...
 *
 * @example
 * для html <div class="some-class-1">
 *     <b>привет!</b>
...
function collectDOMStat(root, obj) {
    ...
    for (var item of root.childNodes) {
        if (item.nodeType === 3) {
            obj.texts++;
        }

        if (item.nodeType === 1) {
            obj.tags[item.tagName] = obj.tags.hasOwnProperty(item.tagName) ? ++ obj.tags[item.tagName] : 1;

            var classList = item.classList;

            for (let i = 0; i < classList.length; i++) {
                obj.classes[classList[i]] = obj.classes.hasOwnProperty(classList[i]) ? ++ obj.classes[classList[i]] : 1;
            }
    ...
    var observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            ...
            for (let i = 0; i < mutation.addedNodes.length; i++) {
                insertArray.push(mutation.addedNodes[i]);
            }

            for (let i = 0; i < mutation.removedNodes.length; i++) {
                removeArray.push(mutation.removedNodes[i]);
            }
